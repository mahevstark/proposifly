  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Admins</h1>
          <p className="text-sm text-white/40 mt-1">Manage admin access</p>
        </div>
        <button
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] overflow-hidden">
        <div className="px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-white">Active Admins</h2>
            <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold text-white/35 bg-white/[0.05]">{admins.length}</span>
          </div>
        </div>
        <div className="divide-y divide-white/[0.04]">
          {admins.map((a, i) => (
            <div key={a.id} className="flex items-center justify-between px-5 py-4 hover:bg-white/[0.02] transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-white">{a.name || "No Name"}</p>
