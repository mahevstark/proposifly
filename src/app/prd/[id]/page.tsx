...
export default function PRDPage() {
  ...
  // Re-number enabled phases for display
  const numberedPhases = phases.map((p, _i) => {
    let displayNum = 0;
    let count = 0;
    for (let j = 0; j <= _i; j++) {
      if (phases[j].enabled) count++;
    }
    if (p.enabled) displayNum = count;
    return { ...p, displayNumber: displayNum };
  });
    ...
    try {
      const res = await fetch("/api/prd", {
        ...
        body: JSON.stringify({
          ...
          timeline,
          clientName,
          phases: phases.filter((p) => p.enabled).map((p, i) => ({
            number: i + 1,
            title: p.title,
            enabled: true,
          })),
  ...
  if (authLoading || fetchingProposal) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 text-center text-vscode-text-muted">
        Loading...
      </div>
    );
  }
  ...
  if (!proposal) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8 text-center">
        ...
        <Link href="/history" className="btn-primary text-sm px-4 py-2">
          Back to History
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      <div>
        <Link href="/history" className="text-vscode-primary text-sm hover:underline mb-2 inline-block">
          &larr; Back to History
        </Link>
        <h1 className="text-2xl font-bold text-white">PRD Generator</h1>
        <p className="text-vscode-text-muted text-sm mt-1">
          Generate a Project Requirements Document from your accepted proposal.
        </p>
      </div>
      ...
      <div className="bg-vscode-sidebar border border-vscode-border rounded-lg p-4">
        ...
        <p className="text-vscode-text text-sm line-clamp-3">
          {proposal.proposal_text.slice(0, 200)}...
        </p>
      </div>
                ...
                <span className="text-white text-sm">
                  {phase.enabled && (
                    <span className="text-vscode-primary font-semibold mr-2">
                      Phase {numberedPhases[index].displayNumber}:
                    </span>
                  )}
                  {phase.title}
                </span>
      ...
      {prdText && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => handleCopy(prdText)}
              className="text-sm px-4 py-1.5 rounded bg-vscode-primary/20 text-vscode-primary hover:bg-vscode-primary/30 transition-colors"
            >
              {copied ? "Copied!" : "Copy PRD"}
            </button>
          </div>
          <div className="bg-vscode-bg border border-vscode-border rounded-lg p-6 text-vscode-text text-sm leading-relaxed whitespace-pre-wrap">
            {prdText}
          </div>
        </div>
      )}
